...
def get_loader(loader_name: str, file_path: str, loader_kwargs: Dict = None):
    ...
    try:
        ...
        if loader_name in ["UnstructuredLightPipeline"]:
            ...
            # 对UnstructuredLightPipeline进行特殊处理，不传递file_path给构造函数
...
class KnowledgeFile:
    ...
    async def file2docs(self, refresh: bool = False):
        if self.docs is None or refresh:
            ...
            # 对UnstructuredLightPipeline进行特殊处理，调用run_pipeline而不是load
            if self.document_loader_name == "UnstructuredLightPipeline":
                self.docs = await loader.run_pipeline(self.filepath, ['unstructured'])
            else:
                self.docs = loader.load()

        return self.docs

    async def docs2texts(
            self,
        ...
        docs = docs or await self.file2docs(refresh=refresh)
        if not docs:
            return []
    ...
    async def file2text(
        ...
        if self.splited_docs is None or refresh:
            docs = await self.file2docs()

            self.splited_docs = await self.docs2texts(docs=docs,
