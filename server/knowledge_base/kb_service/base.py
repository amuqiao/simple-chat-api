...
class EmbeddingsFunAdapter(Embeddings):
    ...
    async def aembed_documents(self, texts: List[str]) -> List[List[float]]:
        result = await aembed_texts(texts=texts, embed_model=self.embed_model, to_query=False)
        if result.data is None:
            raise ValueError(f"嵌入模型 {self.embed_model} 无法生成有效的向量: {result.msg}")
        return normalize(result.data).tolist()

    async def aembed_query(self, text: str) -> List[float]:
        result = await aembed_texts(texts=[text], embed_model=self.embed_model, to_query=True)
        if result.data is None:
            raise ValueError(f"嵌入模型 {self.embed_model} 无法生成有效的查询向量: {result.msg}")
        query_embed = result.data[0]
        query_embed_2d = np.reshape(query_embed, (1, -1))  # 将一维数组转换为二维数组
        normalized_query_embed = normalize(query_embed_2d)
        return normalized_query_embed[0].tolist()  # 将结果转换为一维数组并返回
